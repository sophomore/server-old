    ...
    if unit == 1 or unit == 3:
        ...
        while current<=end:
            if unit == 2:
                ...
                ordermenus = db.query(OrderMenu).join(Order).filter(current<=Order.time, Order.time<=quarter_month).all()
            ...
            for ordermenu in ordermenus:
                if ordermenu.menu_id in menus:
                    ...
                    reulst[menu_name[ordermenu.menu_id]][count_for_result]['count'] += 1
...
def bar_chart(startDate,endDate,menus,unit):
    ...
    if unit == 1 or unit == 3:
        ...
        while current<=end:
            ...
            for ordermenu in ordermenus:
                if ordermenu.menu_id in menus:
                    reulst[count_for_result]['count'] += 1
